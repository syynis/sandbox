...
use sandbox::{
    ...
    level::{
        from_world_pos,
        placement::{StorageAccess, TileModification, TileUpdateEvent},
...
fn setup_tiles(mut tile_placer: StorageAccess, mut once: Local<bool>) {
    if !(*once) {
        for x in 10..16u32 {
...
fn clear(keys: Res<Input<KeyCode>>, mut tile_placer: StorageAccess) {
